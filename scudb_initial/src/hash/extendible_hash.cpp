 ...
 */
    ...
    int ExtendibleHash<K, V>::GetGlobalDepth() const{
        lock_guard<mutex> lock(ExtendibleHash<K, V>::latch); //lock during its life period
        return globalDepth;
    }
 ...
 */
    ...
    int ExtendibleHash<K, V>::GetLocalDepth(int bucket_id) const {
        ...
        if (buckets[bucket_id]) {
            ...
            if (buckets[bucket_id]->hMap.size() == 0) return -1;
 ...
 */
    ...
    int ExtendibleHash<K, V>::GetNumBuckets() const{
        lock_guard<mutex> lock(ExtendibleHash<K, V>::latch);
        return bucketNum;
    }
 ...
 */
    ...
    bool ExtendibleHash<K, V>::Find(const K &key, V &value) {
        ...
        if (buckets[idx]->hMap.find(key) != buckets[idx]->hMap.end()) {
            value = buckets[idx]->hMap[key];
            return true;
        }
        return false;
    }
 ...
 */
    ...
    bool ExtendibleHash<K, V>::Remove(const K &key) {
        ...
        if (tmp->hMap.find(key) == tmp->hMap.end()) {
            return false;
        }
        tmp->hMap.erase(key);
        return true;
 ...
 */
    ...
    void ExtendibleHash<K, V>::Insert(const K &key, const V &value) {
        ...
        while (true) {
            ...
            if (tmp->hMap.find(key) != tmp->hMap.end() || tmp->hMap.size() < bucketSize) {
                tmp->hMap[key] = value;
                break;
            }
            int mask = (1 << (tmp->localDepth));
            tmp->localDepth++;
            lock_guard<mutex> lck2(latch);
            ...
            for (it = tmp->hMap.begin(); it != tmp->hMap.end(); ) {
                if (HashKey(it->first) & mask) {
                    newBuc->hMap[it->first] = it->second;
                    it = tmp->hMap.erase(it);
                } else it++;
            }
            for (size_t i = 0; i < buckets.size(); i++) {
                if (buckets[i] == tmp && (i & mask))
                    buckets[i] = newBuc;
            }
            idx = getIdx(key);
            tmp = buckets[idx];
